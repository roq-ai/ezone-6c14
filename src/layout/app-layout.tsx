...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiServer } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'admin', 'member', 'moderator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ServerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/servers` : null),
    () => getServers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Server',
      path: isTenantUser ? `/servers/view/${data?.[0]?.id}` : '/servers',
      entity: 'server',
